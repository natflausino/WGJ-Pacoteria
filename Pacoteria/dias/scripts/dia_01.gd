...
func _ready():
	...
	$MadTalk.connect("dialog_advanced", Callable(self, "_on_dialog_advanced"))
...
func _on_dialog_advanced(current_id: String):
	print("ID atual:", current_id)

	if current_id == "6": # ou o ID que você definiu no bloco
		print("Iniciando puzzle de segurança")
		$MadTalk.pause_dialog()
		$Puzzle.visible = true
